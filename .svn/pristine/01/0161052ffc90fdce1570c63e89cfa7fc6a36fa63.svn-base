  ...
  <div>
    <div v-if="showType=='finanialLoan'" key="'finanialLoan'">
      ...
      <el-table :data="tableData" ref="multipleTable" @selection-change="handleSelectionChange" :row-key="'orderId'"
        ...
        <el-table-column type="index" width="55" label="序号"
                         :index="$api.IndexMethod(formData.pageIndex,formData.pageSize)"></el-table-column>
        ...
        <el-table-column label="操作" align='center'>
          <template slot-scope="scope">
            ...
            <el-button type="text" v-show="scope.row.matchState==='1'&&scope.row.dealState==='0'"
                       @click="recovery(scope.row)">回收
            </el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <div class="pull-right mg-t20">
        <jb-pagination :totalCount="totalCount" :pageIndex="formData.pageIndex" :pageSize="formData.pageSize"
                       @getList="getWorkList"></jb-pagination>
      </div>
          ...
          <el-form-item label="分配客服：">
            <ul>
              <li v-for="(item,key) in formData.matchOrderItem" :key="key" class="mg-b15">
                ...
                <el-input type="number" v-model.number="item.matchNum" @blur.prevent="()=> checkNum(item.matchNum)"
                          placeholder="分配数量" class="width200"></el-input>
    ...
    <div v-if="showType=='weddingPhoto'" key="'weddingPhoto'">
      ...
      <el-table :data="tableData" ref="multipleTable" @selection-change="handleSelectionChange" :row-key="'orderId'"
        ...
        <el-table-column prop="eventMarketId" label="活动ID" show-overflow-tooltip></el-table-column>
        <el-table-column prop="promotionChannel" label="推广渠道" align='center' show-overflow-tooltip></el-table-column>
        <el-table-column prop="promotionChannelChild" label="二级渠道" align='center' show-overflow-tooltip></el-table-column>
  ...
  export default {
    ...
    created() {
      ...
      if(this.selectRow.id){
        this.formData.taskId=this.selectRow.id;//任务编号
        if(this.selectRow.businessName=="婚纱摄影"){
