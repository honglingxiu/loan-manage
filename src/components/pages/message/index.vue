  ...
  <div>
    ...
    <div v-show="!showEditPage&&!showAddPage&&!showDetailPage">
      <div class="mg15 mg-l0">
        ...
        <el-date-picker v-model="value3" type="daterange" start-placeholder="开始日期" end-placeholder="结束日期"
      ...
      <el-table :data="msgList" stripe border>
        ...
        <el-table-column prop="operation" label="操作">
          <template slot-scope="scope">
            ...
            <el-button type="text" v-show="scope.row.sendStatus == 0"
                       @click.native.prevent="confirm(scope.$index, msgList)">
              删除
            </el-button>
            <el-button @click="editSelectedMsg(scope.row)" type="text"
                       v-show="scope.row.sendStatus == '0'">编辑
            </el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <div class="mg15 mg-r0 pull-right">
        <jb-pagination @getList="getMsgList" :pageIndex="pageIndex" :pageSize="pageSize" :totalCount="totalCount"></jb-pagination>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getMsgList: function (pageIndex,pageSize) {
